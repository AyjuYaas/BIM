...
def decrypt(cipher, key):
    ...
    for i in range(0, len_text, 2):
        if cipher[i].islower():
            plaintext.append((inv_key[0][0] * (ord(cipher[i]) - ord('a')) + inv_key[0][1] * (ord(cipher[i + 1]) - ord('a'))) % 26)
            plaintext.append((inv_key[1][0] * (ord(cipher[i]) - ord('a')) + inv_key[1][1] * (ord(cipher[i + 1]) - ord('a'))) % 26)
